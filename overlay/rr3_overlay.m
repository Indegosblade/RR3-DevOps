...
static BOOL g_supportedBuild = NO;

static const uint8_t kSupportedMachoUUID[16] = {
    0x06, 0xed, 0x5b, 0x50, 0xc4, 0x4c, 0x3b, 0x69,
    0xa5, 0x84, 0x07, 0x01, 0xb8, 0xa8, 0x84, 0x51,
};

static BOOL mappedRange(uintptr_t address, size_t length, vm_prot_t required) {
    ...
    if (result != KERN_SUCCESS || address < region || address - region > regionSize)
        return NO;
    if (length > regionSize - (address - region)) return NO;
    return (info.protection & required) == required;
}

static BOOL validateMainImage(void) {
    ...
    for (uint32_t i = 0; i < header->ncmds; i++) {
        ...
        if (command->cmd == LC_UUID && command->cmdsize >= sizeof(struct uuid_command)) {
            ...
            if (memcmp(uuid->uuid, kSupportedMachoUUID, sizeof(kSupportedMachoUUID)) == 0) {
...
static BOOL readFlag(int idx, uint8_t *value) {
    if (!g_supportedBuild || idx < 0 || idx >= NUM_FLAGS) return NO;
    uint8_t *flag = flagPtr(idx);
    if (!mappedRange((uintptr_t)flag, sizeof(*flag), VM_PROT_READ)) return NO;
    *value = *flag;
    return YES;
}

static BOOL writeFlag(int idx, uint8_t value) {
    if (!g_supportedBuild || idx < 0 || idx >= NUM_FLAGS) return NO;
    uint8_t *flag = flagPtr(idx);
    if (!mappedRange((uintptr_t)flag, sizeof(*flag), VM_PROT_READ | VM_PROT_WRITE)) return NO;
    *flag = value;
    return *flag == value;
}
...
#define TWEAK_VEC_CAP   0x10324f488
#define TWEAK_STRIDE    0x78
#define TWEAK_MAX_ENTRIES 2048
...
static NSString *readSSO(uint8_t *base) {
    if (!mappedRange((uintptr_t)base, 24, VM_PROT_READ)) return @"??";
    ...
    if ((uintptr_t)ptr > 0x100000000ULL && sz < 256 &&
        mappedRange((uintptr_t)ptr, (size_t)sz, VM_PROT_READ))
        return [[NSString alloc] initWithBytes:ptr length:sz
                    encoding:NSUTF8StringEncoding] ?: @"??";
    return @"??";
}

static size_t tweakValueSize(uint32_t type) {
    switch (type) {
        case TW_BOOL: return sizeof(uint8_t);
        case TW_INT: return sizeof(int32_t);
        case TW_FLOAT: return sizeof(float);
        case TW_DOUBLE: return sizeof(double);
        default: return 0;
    }
}

static float readTweakFloat(uint8_t *entry) {
    if (!g_supportedBuild || !mappedRange((uintptr_t)entry, TWEAK_STRIDE, VM_PROT_READ))
        return 0;
    uint32_t type = *(uint32_t *)(entry + TW_OFF_TYPE);
    void *vp = *(void **)(entry + TW_OFF_PTR);
    size_t valueSize = tweakValueSize(type);
    if (!vp || !valueSize || !mappedRange((uintptr_t)vp, valueSize, VM_PROT_READ)) return 0;
    switch (type) {
...
static BOOL writeTweak(uint8_t *entry, float val) {
    if (!g_supportedBuild ||
        !mappedRange((uintptr_t)entry, TWEAK_STRIDE, VM_PROT_READ | VM_PROT_WRITE))
        return NO;
    uint32_t type = *(uint32_t *)(entry + TW_OFF_TYPE);
    void *vp = *(void **)(entry + TW_OFF_PTR);
    size_t valueSize = tweakValueSize(type);
    if (!vp || !valueSize ||
        !mappedRange((uintptr_t)vp, valueSize, VM_PROT_READ | VM_PROT_WRITE))
        return NO;
    switch (type) {
        case TW_BOOL:
...
static BOOL writeTweakVerified(uint8_t *entry, float val) {
    if (!writeTweak(entry, val)) return NO;
...
static BOOL buildTweakIndex(void) {
    if (!g_supportedBuild) return NO;
    uint8_t **bs = (uint8_t **)((uintptr_t)TWEAK_VEC_BASE + g_slide);
    uint8_t **es = (uint8_t **)((uintptr_t)TWEAK_VEC_END + g_slide);
    uint8_t **cs = (uint8_t **)((uintptr_t)TWEAK_VEC_CAP + g_slide);
    if (!mappedRange((uintptr_t)bs, sizeof(*bs) * 3, VM_PROT_READ)) {
        NSLog(@"[RR3] Tweak-vector header is not readable");
        return NO;
    }
    uint8_t *base = *bs, *end = *es;
    uint8_t *capacity = *cs;
    if (!base || !end || !capacity || end < base || capacity < end) {
        NSLog(@"[RR3] Tweak vector not ready or invalid (base=%p end=%p cap=%p)",
            base, end, capacity);
        return NO;
    }
    uintptr_t bytes = (uintptr_t)(end - base);
    if (bytes % TWEAK_STRIDE != 0 || bytes / TWEAK_STRIDE > TWEAK_MAX_ENTRIES ||
        !mappedRange((uintptr_t)base, (size_t)bytes, VM_PROT_READ)) {
        NSLog(@"[RR3] Tweak vector failed alignment, count, or mapping validation");
        return NO;
    }
    g_tweakCount = (int)(bytes / TWEAK_STRIDE);
    ...
    for (int i = 0; i < g_tweakCount; i++) {
        ...
        uint32_t type = *(uint32_t *)(entry + TW_OFF_TYPE);
        if (type > TW_FLOAT || [name isEqualToString:@"??"]) continue;
        if (type != TW_STRING) {
            void *value = *(void **)(entry + TW_OFF_PTR);
            if (!mappedRange((uintptr_t)value, tweakValueSize(type), VM_PROT_READ)) continue;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
    if (ip.section == 0) {
        ...
        sw.onTintColor = ACCENT;
        uint8_t value = 0;
        sw.on = readFlag((int)ip.row, &value) && value != 0;
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)ip {
    ...
    if (ip.section == 0) {
        ...
        uint8_t val = 0;
        BOOL readable = readFlag((int)ip.row, &val);
        NSString *msg = [NSString stringWithFormat:
            @"%@\n\nState: %@\nBSS address: 0x%llx",
            desc, readable ? (val ? @"ON" : @"OFF") : @"unavailable", g_flags[ip.row].va];
...
- (void)flagToggled:(UISwitch *)sw {
    BOOL ok = writeFlag((int)sw.tag, sw.on ? 1 : 0);
...
- (void)allOn {
    for (int i = 0; i < NUM_FLAGS; i++) writeFlag(i, 1);
...
- (void)resetAllTweaks {
    ...
    for (int i = 0; i < NUM_FLAGS; i++) writeFlag(i, 0);
    [_table reloadData];
...
#define TWEAK_INDEX_RETRY_SECONDS 2.0
#define TWEAK_INDEX_MAX_ATTEMPTS 5

static void populateTweakIndex(int attemptsRemaining) {
    if (buildTweakIndex()) {
        dumpTweakables();
        if (g_mainMenu) {
    ...
    if (attemptsRemaining <= 1) {
        NSLog(@"[RR3] Tweak vector did not become ready after %d attempts",
            TWEAK_INDEX_MAX_ATTEMPTS);
        return;
    }
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
        (int64_t)(TWEAK_INDEX_RETRY_SECONDS * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        populateTweakIndex(attemptsRemaining - 1);
    });
}

__attribute__((constructor))
static void rr3_overlay_init(void) {
    g_supportedBuild = validateMainImage();
    if (!g_supportedBuild) {
        NSLog(@"[RR3] Overlay refused: unsupported main executable UUID");
        return;
    }
    initLabels();
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(8.0 * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
        populateTweakIndex(TWEAK_INDEX_MAX_ATTEMPTS);
    });
