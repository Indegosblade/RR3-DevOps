"""
Build patched RR3 IPA: all binary patches + NOP all debug gates.
...
UNSUPPORTED EXPERIMENT. Explicit acknowledgement required.
Usage: python build_debug_v1.py --experimental <original.ipa> <output.ipa>
"""
...
from build_4k_v3 import validate_binary

if len(sys.argv) != 4 or sys.argv[1] != '--experimental':
    print(f"Usage: {sys.argv[0]} --experimental <original.ipa> <output.ipa>")
    print('Refused: this is a known-unstable research builder, not a release path.')
    sys.exit(1)
...
try:
    validate_binary(raw_binary)
except ValueError as exc:
    print(f'Refused: experimental builder requires the verified original final executable: {exc}',
          file=sys.stderr)
    sys.exit(1)
...
for adrp_va, var_va, desc in GATE_ADRP_LOCATIONS:
    ...
    for j in range(1, 9):
        ...
        if kind:
            ...
            print(f"  [{check_foff:#010x}] {desc}: {kind} skip {skip}B -> NOP")
